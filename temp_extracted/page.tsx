...
type ViewKey = 'landing' | 'login' | 'upload' | 'analyzing' | 'checklist';
...
/* ══════════════════════════════════════════════════════════════
   UPLOAD
══════════════════════════════════════════════════════════════ */
function UploadView({ onUpload }: { onUpload: () => void }) {
          ...
          {!file ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={onUpload} className="btn-forest" style={{ fontSize: 12, padding: '9px 24px' }}>
                  Analisar com IA →
                </button>
              </div>
            </div>
...
/* ══════════════════════════════════════════════════════════════
   CHECKLIST
══════════════════════════════════════════════════════════════ */
function ChecklistView({ parties, setParties, formId, addToast }: {
  ...
  const total    = parties.reduce((a, p) => a + p.documents.length, 0);
  const uploaded = parties.reduce((a, p) => a + p.documents.filter(d => d.status === 'uploaded').length, 0);
